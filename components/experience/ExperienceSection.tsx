...
export const ExperienceSection: React.FC<ExperienceSectionProps> = ({
  ...
  const academicCourses = education.relevantCourses ||
    education.courses || [
      ...
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <div className="space-y-4">
          ...
          <p className="text-base sm:text-lg text-neutral-600 dark:text-neutral-400 max-w-2xl leading-relaxed">
            Track record of architecting mission-critical platforms, high-throughput engines, and
            fluid design systems in production environments.
          </p>
        </div>
                ...
                >
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div
                        className={`grid gap-4 ${exp.products.length > 1 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"}`}
                      >
                    ...
                    {/* Technology Tags */}
                    <div className="flex flex-wrap gap-2 pt-2 border-t border-black/4 dark:border-white/6">
                      {exp.technologies.map((tech: string, tIdx: number) => (
                        <span
                          key={tIdx}
                          className="px-2.5 py-1 rounded-lg bg-stone-100 dark:bg-white/4 text-neutral-700 dark:text-neutral-300 font-mono text-xs border border-black/4 dark:border-white/4"
                        >
                          {tech}
                        </span>
                      ))}
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-black/6 dark:border-white/8 pb-6">
                <div className="space-y-1">
                  ...
                  <p className="text-xs sm:text-sm text-neutral-600 dark:text-neutral-400">
                    {education.location || "Sylhet, Bangladesh"} &middot; Class of{" "}
                    {education.period || "2018 – 2023"}
                  </p>
                </div>

                <span className="px-3.5 py-1.5 rounded-full bg-stone-100 dark:bg-white/6 text-neutral-700 dark:text-neutral-300 text-xs sm:text-sm font-semibold font-mono shrink-0 self-start">
                  {education.period || "2018 – 2023"}
                </span>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2.5">
                  {academicCourses.map((course: string, cIdx: number) => (
                    <div
                      key={cIdx}
                      className="p-3 rounded-xl bg-stone-50 dark:bg-white/2 border border-black/4 dark:border-white/6 text-xs font-mono text-neutral-700 dark:text-neutral-300"
                    >
                      {course}
                    </div>
                  ))}
